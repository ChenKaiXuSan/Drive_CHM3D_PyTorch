...
1. **帧级文件缺失（某视角没有该帧npz）**
	- 在 `assemble_view_npz_paths` 中按多视角帧号取交集（common frames）。
	- 只要某视角缺该帧，该帧不会进入融合流程。

2. **帧文件存在但关键点无效（全NaN）**
	- 在 `_fuse_single_person_env` 中检测 `pred_keypoints_3d` 是否整帧无效。
	- 只要任一视角全NaN，当前帧直接 `continue` 跳过，不保存融合结果。

该策略保证了输入完整性，但会减少可用帧数量。

### 7.2 推荐工程策略（更稳健）

在实际采集中，建议使用“降级融合”而不是整帧丢弃：

1. **有效视角数 >= 2：执行融合**
	- 对可用视角继续使用 `median/mean` 融合。
	- 无效视角以 `NaN` 参与掩码，融合时自动忽略。
...
3. **日志与质量标记**
	- 记录每帧缺失视角列表、有效视角数量、是否触发降级。
	- 在后续评估中统计 `missing_view_ratio` 与 `degraded_fusion_ratio`。

该策略能在“单视角偶发缺失”场景下显著提升时序连续性和结果可用率。
